...
event zeek_init()
	...
	event ssl_extension_signature_algorithm(dummy, T, vector([ $HashAlgorithm=4,
	    ...
	    $SignatureAlgorithm=4 ], [ $HashAlgorithm=8, $SignatureAlgorithm=5
	    ], [ $HashAlgorithm=8, $SignatureAlgorithm=6 ], [ $HashAlgorithm=4,
	    $SignatureAlgorithm=1 ], [ $HashAlgorithm=5, $SignatureAlgorithm=1
	    ], [ $HashAlgorithm=6, $SignatureAlgorithm=1 ], [ $HashAlgorithm=2,
	    $SignatureAlgorithm=3 ], [ $HashAlgorithm=2, $SignatureAlgorithm=1
	    ], [ $HashAlgorithm=2, $SignatureAlgorithm=2 ], [ $HashAlgorithm=4,
	    $SignatureAlgorithm=2 ], [ $HashAlgorithm=5, $SignatureAlgorithm=2
	    ], [ $HashAlgorithm=6, $SignatureAlgorithm=2 ]));
	...
	event ssl_extension_psk_key_exchange_modes(dummy, T, vector(1, 0));
	event ssl_extension(dummy, T, 45, "\x02\x01\x00");
	event ssl_extension_key_share(dummy, T, vector(29));
	...
	event ssl_client_hello(dummy, 771, 769, network_time(), "y\x83\x7fU\xe4-\xa6\xd5\x1c\x87\xdc\xa3]`{\xec\x9f\x89\x07}\xba\x90\xe2s\x99\xc3\xf2\x82", "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00",
	    vector(49196, 49200, 159, 52393, 52392, 52394, 49195, 49199, 158,
	    49188, 49192, 107, 49187, 49191, 103, 49162, 49172, 57, 49161,
	    49171, 51, 157, 156, 4866, 4867, 4865, 61, 60, 53, 47, 255), vector(0));
	...
	event ssl_client_hello(dummy, 771, 769, network_time(), ";\x03v\xb1pU\xb2Hf\x8d2\x02\xbb\xe8\xa9\xb90/\xeb\xe9?\xfe;j\xcf\xa1d8", "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00",
	    vector(49196, 49200, 159, 52393, 52392, 52394, 49195, 49199, 158,
	    49188, 49192, 107, 49187, 49191, 103, 49162, 49172, 57, 49161,
	    49171, 51, 157, 156, 4866, 4867, 4865, 61, 60, 53, 47, 255), vector(0));
	event my_finalize_ssl(dummy);
