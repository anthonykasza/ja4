...
event zeek_init()
	...
	event ssl_extension_application_layer_protocol_negotiation(dummy, T, vector(
	    "spdy/3", "spdy/3.1", "http/1.1"));
	event ssl_extension(dummy, T, 16,
	    "\x00\x19\x06spdy/3\x08spdy/3.1\x08http/1.1");
	...
	event ssl_extension_signature_algorithm(dummy, T, vector([ $HashAlgorithm=4,
	    ...
	    ], [ $HashAlgorithm=2, $SignatureAlgorithm=3 ], [ $HashAlgorithm=4,
	...
	event ssl_client_hello(dummy, 771, 769, network_time(), "\xccTd\xd4;\x10F\xf6\x88%\xac\xf4m\x92f2\x89\xd2\xf9\x05\xec\xfah~\x85j\xff\xaa", "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00",
	    vector(49195, 49199, 158, 52244, 52243, 49162, 49161, 49171, 49172,
	    49159, 49169, 51, 50, 57, 156, 47, 53, 10, 5, 4), vector(0));
