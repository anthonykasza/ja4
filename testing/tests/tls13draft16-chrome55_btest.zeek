# @TEST-EXEC: zeek $PACKAGE %INPUT >output
# @TEST-EXEC: cat ja4.log | zeek-cut ja4 o r ro grease_hash > ja4.filtered
# @TEST-EXEC: btest-diff ja4.filtered
# @TEST-EXEC: btest-diff output

event my_finalize_ssl(dummy: connection)
	{
	hook SSL::finalize_ssl(dummy);
	}

event zeek_init()
	{
	local dummy: connection = [ $uid="UUIIDD", $start_time=network_time(),
	    $id=[ $orig_h=1.1.1.1, $orig_p=1/tcp, $resp_h=2.2.2.2,
	    $resp_p=2/tcp ], $orig=[ $size=0, $state=0, $flow_label=0 ],
	    $resp=[ $size=0, $state=0, $flow_label=0 ], $duration=0msec,
	    $service=set("SSL"), $history="" ];

	event ssl_extension(dummy, T, 19018, "");
	event ssl_extension(dummy, T, 65281, "\x00");
	event ssl_extension(dummy, T, 23, "");
	event ssl_extension(dummy, T, 35, "");
	event ssl_extension_signature_algorithm(dummy, T, vector([ $HashAlgorithm=8,
	    $SignatureAlgorithm=6 ], [ $HashAlgorithm=6, $SignatureAlgorithm=1
	    ], [ $HashAlgorithm=6, $SignatureAlgorithm=3 ], [ $HashAlgorithm=8,
	    $SignatureAlgorithm=5 ], [ $HashAlgorithm=5, $SignatureAlgorithm=1
	    ], [ $HashAlgorithm=5, $SignatureAlgorithm=3 ], [ $HashAlgorithm=8,
	    $SignatureAlgorithm=4 ], [ $HashAlgorithm=4, $SignatureAlgorithm=1
	    ], [ $HashAlgorithm=4, $SignatureAlgorithm=3 ], [ $HashAlgorithm=2,
	    $SignatureAlgorithm=1 ], [ $HashAlgorithm=2, $SignatureAlgorithm=3
	    ]));
	event ssl_extension(dummy, T, 13, "\x00\x16\x08\x06\x06\x01\x06\x03\x08\x05\x05\x01\x05\x03\x08\x04\x04\x01\x04\x03\x02\x01\x02\x03");
	event ssl_extension(dummy, T, 5, "\x01\x00\x00\x00\x00");
	event ssl_extension(dummy, T, 18, "");
	event ssl_extension_application_layer_protocol_negotiation(dummy, T, vector(
	    "h2", "http/1.1"));
	event ssl_extension(dummy, T, 16, "\x00\x0c\x02h2\x08http/1.1");
	event ssl_extension(dummy, T, 30032, "");
	event ssl_extension(dummy, T, 11, "\x01\x00");
	event ssl_extension_key_share(dummy, T, vector(43690, 29));
	event ssl_extension(dummy, T, 40, "\x00)\xaa\xaa\x00\x01\x00\x00\x1d\x00 \xf2/4'\xa8\x10\xe4\xa3\xfb\xde\xd2\x923>_\x92\x83\x1a\x7f7\xd3\xfe\xec\xf3\xb6[\x04\x9aH\xb4\xdc:");
	event ssl_extension_supported_versions(dummy, T, vector(2570, 32526, 771, 770,
	    769));
	event ssl_extension(dummy, T, 43,
	    "\x0a\x0a\x0a\x7f\x0e\x03\x03\x03\x02\x03\x01");
	event ssl_extension(dummy, T, 10, "\x00\x08\xaa\xaa\x00\x1d\x00\x17\x00\x18");
	event ssl_extension(dummy, T, 24, "\x00\x0a\x01\x02");
	event ssl_extension(dummy, T, 2570, "\x00");
	event ssl_extension(dummy, T, 21, "");
	event ssl_client_hello(dummy, 771, 769, network_time(), "\x04\xf9\xc5\xb3\"\xb9\x80\xda$\xd3\x1f\x8d\xa7&\xf4!7\xbe,i\x9f\xf8\xb3YB/\xe2\xea", "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00",
	    vector(27242, 49195, 49199, 49196, 49200, 52393, 52392, 52244,
	    52243, 49161, 49171, 49162, 49172, 156, 157, 47, 53, 10), vector(0));
	event my_finalize_ssl(dummy);
	event ssl_extension_key_share(dummy, F, vector(29));
	event ssl_extension(dummy, F, 40, "\x00\x1d\x00 O\x13\xc5\xd0t^\x88;\xc2\x7f}!\x95\xde\x1fE\xa1\xae\xa1cd\x1d\x85\xc9\xaa\xc2$\x99,s\xbb\x11");
	event ssl_extension(dummy, T, 27242, "");
	event ssl_extension(dummy, T, 65281, "\x00");
	event ssl_extension(dummy, T, 23, "");
	event ssl_extension(dummy, T, 35, "");
	event ssl_extension_signature_algorithm(dummy, T, vector([ $HashAlgorithm=8,
	    $SignatureAlgorithm=6 ], [ $HashAlgorithm=6, $SignatureAlgorithm=1
	    ], [ $HashAlgorithm=6, $SignatureAlgorithm=3 ], [ $HashAlgorithm=8,
	    $SignatureAlgorithm=5 ], [ $HashAlgorithm=5, $SignatureAlgorithm=1
	    ], [ $HashAlgorithm=5, $SignatureAlgorithm=3 ], [ $HashAlgorithm=8,
	    $SignatureAlgorithm=4 ], [ $HashAlgorithm=4, $SignatureAlgorithm=1
	    ], [ $HashAlgorithm=4, $SignatureAlgorithm=3 ], [ $HashAlgorithm=2,
	    $SignatureAlgorithm=1 ], [ $HashAlgorithm=2, $SignatureAlgorithm=3
	    ]));
	event ssl_extension(dummy, T, 13, "\x00\x16\x08\x06\x06\x01\x06\x03\x08\x05\x05\x01\x05\x03\x08\x04\x04\x01\x04\x03\x02\x01\x02\x03");
	event ssl_extension(dummy, T, 5, "\x01\x00\x00\x00\x00");
	event ssl_extension(dummy, T, 18, "");
	event ssl_extension_application_layer_protocol_negotiation(dummy, T, vector(
	    "h2", "http/1.1"));
	event ssl_extension(dummy, T, 16, "\x00\x0c\x02h2\x08http/1.1");
	event ssl_extension(dummy, T, 30032, "");
	event ssl_extension(dummy, T, 11, "\x01\x00");
	event ssl_extension_key_share(dummy, T, vector(60138, 29));
	event ssl_extension(dummy, T, 40, "\x00)\xea\xea\x00\x01\x00\x00\x1d\x00 \xcfa;\xdf\xf0\xde\x7f\xa81\x8c\x86\x1c\xe9\xbf\xdd\x98q\x84\xcd\x97\xf5\xff)/ \xb7\xfc\x07\xf9\xac\xdeq");
	event ssl_extension_supported_versions(dummy, T, vector(6682, 32526, 771, 770,
	    769));
	event ssl_extension(dummy, T, 43,
	    "\x0a\x1a\x1a\x7f\x0e\x03\x03\x03\x02\x03\x01");
	event ssl_extension(dummy, T, 10, "\x00\x08\xea\xea\x00\x1d\x00\x17\x00\x18");
	event ssl_extension(dummy, T, 24, "\x00\x0a\x01\x02");
	event ssl_extension(dummy, T, 60138, "\x00");
	event ssl_extension(dummy, T, 21, "");
	event ssl_client_hello(dummy, 771, 769, network_time(), "\x16\x1a\xab\x14\xfcd\x9e\x07\x0d\xb7\xdf\xb0s\x0c!\xe6\xb0\xb6~\xfd\xf8\xc4;\xbfI\xcd\xe7y", "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00",
	    vector(19018, 49195, 49199, 49196, 49200, 52393, 52392, 52244,
	    52243, 49161, 49171, 49162, 49172, 156, 157, 47, 53, 10), vector(0));
	event my_finalize_ssl(dummy);
	event ssl_extension_key_share(dummy, F, vector(29));
	event ssl_extension(dummy, F, 40, "\x00\x1d\x00 D\xf5p\x92\x13v n\xe0\xf8\xd1.\xe5\xbb\x8b5T\xebx\x94s\xb4\xe4\xcf\x82)\xf6\xedZ\xf6\x16N");
	}
