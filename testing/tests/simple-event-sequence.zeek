...
event zeek_init()
	...
	event ssl_extension_signature_algorithm(dummy, T, vector([ $HashAlgorithm=0x01,
	    $SignatureAlgorithm=0x02 ], [ $HashAlgorithm=0xaa,
	    $SignatureAlgorithm=0xaa ]));
	event ssl_extension(dummy, T, 0xaaaa, "GREASE");
	event ssl_extension(dummy, T, 0xaaaa, "GREASE");
	event ssl_extension(dummy, T, SSL::SSL_EXTENSION_SUPPORTED_VERSIONS, "");
	event ssl_extension_supported_versions(dummy, T, vector(0xaaaa, 0x0));
	event ssl_extension_psk_key_exchange_modes(dummy, T, vector(0x2a));
	event ssl_extension_key_share(dummy, T, vector(0xaaaa));
	event ssl_client_hello(dummy, 0xabcd, 0xabce, double_to_time(0.0),
	    "client_rand", "session_id", vector(0xaaaa, 0x2223, 0x2a2a), vector(
	    0x4444, 0x4445, 0xaaaa));
