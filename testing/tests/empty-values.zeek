...
event zeek_done()
	...
	local ch: JA4::ClientHello = [ $version=0xffff, $cipher_suites=( vector() as
	    vector of count ), $compression_methods=( vector() as vector of
	    count ), $extension_codes=( vector() as vector of count ),
	    $alpns=( vector() as vector of string ), $signature_algos=( vector() as
	    vector of count ), $sni=( vector() as vector of string ),
